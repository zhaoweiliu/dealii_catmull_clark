...
private:
    ...
    void   solve(const bool first_load_step);
    ...
    double b,A;
    double psi_1 = 0.001,psi_2 = 1, radius;
    ...
    const unsigned int max_load_step = 300;
...
void Nonlinear_shell<dim, spacedim> ::run()
{
    ...
    for (unsigned int step = 0; step < max_load_step; ++step) {
        ...
        std::cout << "pressure_load = " << lambda * reference_pressure << "(N)" <<std::endl;

        vtk_plot("cap_09_ar_b_"+std::to_string(step)+".vtu", dof_handler, mapping_collection, vec_values, present_solution,Vector<double>(), lambda * reference_pressure);
    }
}
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
             ...
             ++q_point)
        {
            ...
            for (unsigned int r_shape = 0; r_shape < dofs_per_cell; ++r_shape) {
                ...
                if(r_shape%3 == 0){
                    cell_external_force_rhs[r_shape] += 0.1 *  fe_values.JxW(q_point); //  f^ext;
                }

                // double tol = 1e-9;
                //  for (unsigned int ivert = 0; ivert < GeometryInfo<dim>::vertices_per_cell; ++ivert){
                //     if (cell->vertex(ivert)[0] == 0 && cell->vertex(ivert)[1] == 0 && r_shape%3 == 2){
                //         double shape_val;
                //         switch (ivert) {
                //             case 0:
                //                 shape_val = dof_handler.get_fe(cell->active_fe_index()).shape_value(r_shape, {0,0});
                //                 break;
                //             case 1:
                //                 shape_val = dof_handler.get_fe(cell->active_fe_index()).shape_value(r_shape, {1,0});
                //                 break;
                //             case 2:
                //                 shape_val = dof_handler.get_fe(cell->active_fe_index()).shape_value(r_shape, {0,1});
                //                 break;
                //             case 3:
                //                 shape_val = dof_handler.get_fe(cell->active_fe_index()).shape_value(r_shape, {1,1});
                //                 break;
                //             default:
                //                 shape_val = 0;
                //                 break;
                //         }
                //          cell_external_force_rhs(r_shape) += - reference_pressure * shape_val; //f_z point load
                //     }
                //  }
        ...
        for (unsigned int ivert = 0; ivert < GeometryInfo<dim>::vertices_per_cell; ++ivert){
            if (cell->vertex(ivert)[0] == 0 && cell->vertex(ivert)[1] == 0 && load_added == false){
                ...
                //             break;
                //         }
                //     cell_external_force_rhs[idof] = - reference_pressure * shape_val;
                for(unsigned int idof = 0; idof < dofs_per_cell; ++idof ){
                    double shape_val;
                    if (idof%3 == 2){
                    switch (ivert) {
                    ...
                    cell_external_force_rhs[idof] = - reference_pressure * shape_val;
                    // std::cout << "cell_id = "<< cell->id() << " idof = "<< idof <<" global_id = "<< local_dof_indices[idof]<< " shape_val = " << shape_val << std::endl;
        ...
        internal_force_rhs.add(local_dof_indices, cell_internal_force_rhs);
        external_force_rhs.add(local_dof_indices, cell_external_force_rhs);
        // external_force_rhs[140] = -reference_pressure;
        tangent_matrix.add(local_dof_indices, local_dof_indices, cell_tangent_matrix); 
    }//loop over cells
    double sum_load = 0.0;
...
Triangulation<dim,spacedim> set_mesh( std::string type )
{
    ...
    {
         std::string mfile = "/Users/zhaoweiliu/Documents/geometries/cap_09.msh";
        GridIn<2,3> grid_in;
