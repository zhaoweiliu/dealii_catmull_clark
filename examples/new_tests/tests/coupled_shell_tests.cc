...
//    am_contrav[1][0] = am_contrav[0][1];
    return invert(transpose(am_cov));
}
...
//    vtkSmartPointer<vtkDoubleArray> u_i = vtkDoubleArray::New();
...
//    u_i->SetNumberOfComponents(6);
//    u_i->SetName("u_,i");
//    u_i->SetComponentName(0, "x/xi");
//    u_i->SetComponentName(1, "x/eta");
//    u_i->SetComponentName(2, "y/xi");
//    u_i->SetComponentName(3, "y/eta");
//    u_i->SetComponentName(4, "z/xi");
//    u_i->SetComponentName(5, "z/eta");
...
//            std::string mfile = "cylinder_coarse.msh";
//            GridIn<2,3> grid_in;
//            grid_in.attach_triangulation(mesh);
//            std::ifstream file(mfile.c_str());
//            Assert(file, ExcFileNotOpen(mfile.c_str()));
//            grid_in.read_msh(file);
    ...
    ofstream output_file("roof_refined.vtk");
...
//    elastic_tensor[2][0][2][0] = 25.6e9;
    ...
    dielectric_tensor[0][0] = 1.306e-7;
    dielectric_tensor[1][1] = 1.306e-7;
    dielectric_tensor[2][2] = 1.151e-7;
    ...
    double area = 0;
    double curvature = 0;
    bool load_1 = false, load_2 = false;
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
        const unsigned int dofs_per_cell = cell->get_fe().dofs_per_cell;
        local_dof_indices.resize(dofs_per_cell);
             ...
             ++q_point)
        {
            ...
            Tensor<2, dim, Tensor<1,spacedim>> da_cov; // a_{i,j} = x_{,ij} , i,j = 1,2,3
...
//            std::cout << da_cov[0][0] << std::endl;
//            std::cout << da_cov[0][1] << std::endl;
//            std::cout << da_cov[1][1] << std::endl;
...
//                std::cout << std::endl;
//            }
            auto qpt = q_collection[cell->active_fe_index()].get_points()[q_point];
            Tensor<2,spacedim> da;
            Tensor<2, dim, Tensor<1,spacedim>> dda;
            for (unsigned int ishape = 0; ishape < dofs_per_cell/spacedim; ++ishape){
//                double shape = cell->get_fe().shape_value(ishape*spacedim, qpt);
                Tensor<1,dim> shape_der = cell->get_fe().shape_grad(ishape*spacedim, qpt);
                Tensor<2,dim> shape_der2 = cell->get_fe().shape_grad_grad(ishape*spacedim, qpt);
                for (unsigned int id = 0; id < spacedim; ++id) {
                    da[0][id] += shape_der[0] * vec_values[local_dof_indices[ishape*spacedim+id]];
                    da[1][id] += shape_der[1] * vec_values[local_dof_indices[ishape*spacedim+id]];
                    dda[0][0][id] += shape_der2[0][0] * vec_values[local_dof_indices[ishape*spacedim+id]];
                    dda[0][1][id] += shape_der2[0][1] * vec_values[local_dof_indices[ishape*spacedim+id]];
                    dda[1][0][id] += shape_der2[1][0] * vec_values[local_dof_indices[ishape*spacedim+id]];
                    dda[1][1][id] += shape_der2[1][1] * vec_values[local_dof_indices[ishape*spacedim+id]];
                }
            }
            da[2] = a_cov[2];
            auto a_contra = invert(transpose(a_cov));
//            auto a_contra = covariant_to_contravariant(da);

//            double curvature_1,curvature_2 = 0;
//            Tensor<2, spacedim> curvature_tensor;
//            for (unsigned ii = 0; ii < spacedim; ++ii) {
//                for (unsigned jj = 0; jj < spacedim; ++jj) {
//                    for(unsigned int alpha = 0; alpha < dim; ++alpha){
//                        curvature_tensor[ii][jj] -= 1./detJ * (cross_product_3d(da_cov[0][alpha], a_cov[1]) + cross_product_3d(a_cov[0], da_cov[1][alpha]))[ii] * a_contra[alpha][jj];
//                    }
//                }
//            }
//
//            curvature_1 = - 1./detJ * scalar_product(cross_product_3d(da_cov[0][0], a_cov[1]) + cross_product_3d(a_cov[0], da_cov[1][0]), a_contra[0]);
//            curvature_2 = - 1./detJ * scalar_product(cross_product_3d(da_cov[0][1], a_cov[1]) + cross_product_3d(a_cov[0], da_cov[1][1]), a_contra[1]);
//
...
//                    }
//                }
                Tensor<1,dim> shape_der = cell->get_fe().shape_grad(i_shape*spacedim, qpt);
                Tensor<2,dim> shape_der2 = cell->get_fe().shape_grad_grad(i_shape*spacedim, qpt);
...
//                            membrane_strain[ii][jj][id] += 0.5 * (da[ii][id] * shape_der[jj] + da[jj][id] * shape_der[ii]);
...
//                            bending_strain[ii][jj][id] += (- shape_der2[ii][jj] * da[2][id] + (shape_der[0] * cross_product_3d(dda[ii][jj], da[1])[id] + shape_der[1] * cross_product_3d(da[0], dda[ii][jj])[id])/detJ + scalar_product(da[2], dda[ii][jj]) * (shape_der[0] * cross_product_3d(da[1], da[2])[id] + shape_der[1] * cross_product_3d(da[2], da[0])[id])/detJ);
                        }
//                        std::cout << "membrane strain_"<< ii << jj <<" = "<<membrane_strain[ii][jj] <<"\n";
//                        std::cout << "bending strain_"<< ii << jj <<" = " << bending_strain[ii][jj] <<"\n";
                    }
                }
            ...
            for (unsigned int j_node = 0; j_node < dofs_per_cell/spacedim; ++j_node)
            {
                    ...
                    for(unsigned int jj = 0; jj < dim ; ++jj)
                        for(unsigned int kk = 0; kk < dim ; ++kk)
                            for(unsigned int ll = 0; ll < dim ; ++ll)
                                for (unsigned int id = 0; id < spacedim; ++id) {
                                    hn[kk][ll][id] += c1 * H_tensor[ii][jj][kk][ll] * bn_vec[j_node][ii][jj][id];
                                    hm[kk][ll][id] += c2 * H_tensor[ii][jj][kk][ll] * bm_vec[j_node][ii][jj][id];
                                }
                ...
                // uniform load
                if (type == "p") {
                    cell_force_rhs(j_node*spacedim + 2) += -90 * fe_values.shape_value(j_node*spacedim + 2 , q_point)* fe_values.JxW(q_point); // f_z = 1

                }else if (type == "s" ){
                    cell_force_rhs(j_node*spacedim) += 1 * fe_values.shape_value(j_node*spacedim , q_point)* fe_values.JxW(q_point); // f_y = 1
                }else if(type == "c" ){
                    // uniform load
//                    cell_force_rhs(j_node*spacedim + 2) += 1 * fe_values.shape_value(j_node*spacedim +2 , q_point)* fe_values.JxW(q_point); // f_y = 1
                }
            }// loop over nodes (j)
...
//                if (cell->vertex(ivert)[0] == 0 || cell->vertex(ivert)[0] == 100 || cell->vertex(ivert)[1] == 0 || cell->vertex(ivert)[1] == 100 )
                if (cell->vertex(ivert)[0] == 0 || cell->vertex(ivert)[0] == 50
//                    || cell->vertex(ivert)[1] > 16 || cell->vertex(ivert)[1] < -16
                    )
                {
...
//        vtk_plot("cylindrical_laplace_eigen_solution_"+std::to_string(i)+".vtu", dof_handler, mapping_collection, vec_values, solution_disp ,l_eigenvectors[i]);
//    }
    ...
    ReductionControl reduction_control_K(4000, 1.0e-12, 1.0e-9);
...
//    }
//
//    SolverControl            ela_solver_control(3000, 1e-12);
//    SolverCG<Vector<double>> ela_solver(ela_solver_control);
//    ela_solver.solve(stiffness_matrix, solution_disp, force_rhs, PreconditionIdentity());
//    std::cout << "   " << ela_solver_control.last_step()
//    << " CG iterations needed to obtain convergence." << std::endl;
    
    if(type == "p")
        vtk_plot("plate_uniform_solution.vtu", dof_handler, mapping_collection, vec_values, solution_disp);
    if(type == "s")
...
//                coupled_system_matrix.set(jdof, fix_dof_indices[idof], 0);
    ...
    for (unsigned int i = 0; i < eigenvalues.size(); ++i) {
        ...
        if (type == "c") {
            vtk_plot("cylinder_eigen_solution_"+std::to_string(i)+".vtu", dof_handler, mapping_collection, vec_values, eigenvectors[i]);
        }else if (type == "s"){
            vtk_plot("hemisphere_eigen_solution_"+std::to_string(i)+".vtu", dof_handler, mapping_collection, vec_values, eigenvectors[i]);
        }else if (type == "p"){
            vtk_plot("plate_eigen_solution_"+std::to_string(i)+".vtu", dof_handler, mapping_collection, vec_values, eigenvectors[i]);
        }
    }
