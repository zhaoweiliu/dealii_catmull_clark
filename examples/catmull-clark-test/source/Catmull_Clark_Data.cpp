...
distribute_catmull_clark_dofs(hp::DoFHandler &dh)
{
  std::make_shared<Catmull_Clark<dim, spacedim>> catmull_clark(
    dh.get_triangulation());
  catmull_clark->cell_patch_vector = catmull_clark->cell_patches();
...
Catmull_Clark<dim, spacedim>::Catmull_Clark(
  const Triangulation<dim, spacedim> &tri)
  : dof_handler(tri)
{
       ...
       ++cell)
    {
      ...
      if (iter_fe_valence != map_valence_to_fe_indices.end())
      ...
      else
        {
          ...
          fe_collection.push_back(FE_Catmull_Clark<dim, spacedim>(
            valence, shared_ptr<Catmull_Clark<dim, spacedim>>(this)));
...
Catmull_Clark<dim, spacedim>::cell_patches()
{
...
std::map<unsigned int,
         typename hp::DoFHandler<dim, spacedim>::active_cell_iterator>
Catmull_Clark<dim, spacedim>::ordering_cells_in_patch(
...
Catmull_Clark<dim, spacedim>::new_dofs_for_cells()
{
  std::vector<std::vector<types::global_dof_index>> global_dof_vector_vector(
      ...
      switch (cells.size())
        {
          ...
          case 6:
            {
              ...
              global_dof_indices[11] =
                get_neighbour_dofs(cells[0], cells[4])[0];
          ...
          case 9:
            {
              ...
              global_dof_indices[12] =
                get_neighbour_dofs(cells[0], cells[5])[0];
              global_dof_indices[15] =
                get_neighbour_dofs(cells[0], cells[6])[0];
          ...
          default:
            ...
            for (unsigned int i = 0; i < 4; ++i)
              {
                ...
                for (unsigned int icell = 0; icell < 2; ++icell)
                  {
                    std::vector<types::global_dof_index> icell_dof_indices(4,
                                                                           0);
                    icell_dof_indices.resize(
                      cells[icell + 1]->get_fe().dofs_per_cell);
            ...
            global_dof_indices[5] =
              cell_dof_indices[next_vertices(ex_vertex_index)[0]];
            global_dof_indices[4] =
              cell_dof_indices[next_vertices(ex_vertex_index)[1]];
            global_dof_indices[3] =
              cell_dof_indices[next_vertices(ex_vertex_index)[2]];
            ...
            for (int ic = 1; ic < valence - 1; ++ic)
              {
                ...
                int dia_dof_id = get_diagonal_dof_id_to_ex(cells[0],
                                                           cells[ic],
                                                           ex_vertex_index);
            ...
            dof_pairs[2] =
              get_neighbour_dofs(cells[valence + 3], cells[valence]);
            dof_pairs[3] =
              get_neighbour_dofs(cells[valence + 1], cells[valence]);
            dof_pairs[4] = get_neighbour_dofs(cells[0], cells[valence + 3]);
            dof_pairs[5] =
              get_neighbour_dofs(cells[valence - 1], cells[valence + 4]);
            ...
            std::vector<types::global_dof_index> new_global_dof_indices(
              2 * valence + 8, 0);
            ...
            if (valence == 3)
            ...
            else
              {
                ...
                for (unsigned int i = 0; i < 2 * valence - 7; ++i)
                  {
                    new_global_dof_indices[i + 8] =
                      global_dof_indices[2 * valence - i];
                  }
              }
            ...
            new_global_dof_indices[2 * valence + 4] =
              global_dof_indices[2 * valence + 7];
            new_global_dof_indices[2 * valence + 3] =
              global_dof_indices[2 * valence + 6];
            new_global_dof_indices[2 * valence + 2] =
              global_dof_indices[2 * valence + 5];
            new_global_dof_indices[2 * valence + 1] =
              global_dof_indices[2 * valence + 1];
            new_global_dof_indices[2 * valence + 5] =
              global_dof_indices[2 * valence + 2];
            new_global_dof_indices[2 * valence + 6] =
              global_dof_indices[2 * valence + 3];
            new_global_dof_indices[2 * valence + 7] =
              global_dof_indices[2 * valence + 4];
...
template <int dim, int spacedim>
std::vector<unsigned int> Catmull_Clark<dim, spacedim>::get_neighbour_dofs(
  ...
  typename hp::DoFHandler<dim, spacedim>::active_cell_iterator cell_neighbour)
{
  ...
  if (n_common == 2)
    {
      std::vector<unsigned int> dof_i =
        opposite_face_dofs(ith_dof[0], ith_dof[1]);
      return {static_cast<unsigned int>(
                static_cast<int>(cell_neighbour_indices[dof_i[0]])),
              static_cast<unsigned int>(
                static_cast<int>(cell_neighbour_indices[dof_i[1]]))};
    }
  else if (n_common == 1)
    {
      int dof_i = opposite_vertex_dofs(ith_dof[0]);
      return {static_cast<unsigned int>(
        static_cast<int>(cell_neighbour_indices[dof_i]))};
    }
  else
    {
...
Catmull_Clark<dim, spacedim>::vertex_face_loop(
...
Catmull_Clark<dim, spacedim>::opposite_face_dofs(unsigned int i, unsigned int j)
{
  switch (i)
    {
      case 0:
...
Catmull_Clark<dim, spacedim>::next_vertices(const unsigned int local_vertex_id)
{
  switch (local_vertex_id)
    {
      case 0:
...
template <int dim, int spacedim>
const std::array<unsigned int, 3>
Catmull_Clark<dim, spacedim>::loop_faces(const unsigned int local_face_id)
{
...
Catmull_Clark<dim, spacedim>::opposite_vertices(
...
template <int dim, int spacedim>
const std::array<unsigned int, 2>
Catmull_Clark<dim, spacedim>::faces_not_on_boundary(
...
template <int dim, int spacedim>
const std::array<unsigned int, 4>
Catmull_Clark<dim, spacedim>::verts_id_on_boundary(
  const std::vector<unsigned int> m)
{
  /*
   *       2
...
template <int dim, int spacedim>
unsigned int Catmull_Clark<dim, spacedim>::common_face_local_id(
  typename hp::DoFHandler<dim, spacedim>::active_cell_iterator cell_0,
  typename hp::DoFHandler<dim, spacedim>::active_cell_iterator cell)
{
...
Catmull_Clark<dim, spacedim>::rotated_vertices(const unsigned int local_face_id)
{
  switch (local_face_id)
    {
      case 0:
...
template <int dim, int spacedim>
unsigned int Catmull_Clark<dim, spacedim>::get_diagonal_dof_id_to_ex(
  typename hp::DoFHandler<dim, spacedim>::active_cell_iterator cell_0,
  typename hp::DoFHandler<dim, spacedim>::active_cell_iterator cell_neighbour,
