...
Catmull_Clark<dim,spacedim>::Catmull_Clark(const Triangulation<dim, spacedim> &tri)
:dof_handler(tri)
{   cell_patch_vector = cell_patches();
...
template<int dim, int spacedim>
std::vector<std::set<typename hp::DoFHandler<dim,spacedim>::active_cell_iterator>> Catmull_Clark<dim,spacedim>::cell_patches(){
    std::multimap<unsigned int, typename hp::DoFHandler<dim,spacedim>::active_cell_iterator> vertex_to_cell_map;
    for (typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell = dof_handler.begin_active(); cell != dof_handler.end();++cell )
        for (unsigned int v=0; v < GeometryInfo<2>::vertices_per_cell; ++v)
            vertex_to_cell_map.insert ({cell->vertex_index(v), cell});

    std::vector<std::set<typename hp::DoFHandler<dim,spacedim>::active_cell_iterator>> vector_of_sets;
...
template <int dim, int spacedim>
std::map<unsigned int, typename hp::DoFHandler<dim,spacedim>::active_cell_iterator> Catmull_Clark<dim,spacedim>::ordering_cells_in_patch(typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell, std::set<typename hp::DoFHandler<dim,spacedim>::active_cell_iterator> cells_in_patch){
    ...
    if(cell->at_boundary()==false){
        if(cells_in_patch.size() == 9){
             ...
             *    2-----1
             */
            std::map<int,int> face_to_neighbour,vertex_to_cell;
        ...
        if(cells_in_patch.size() == 6){
            ...
            if(n_boundary_cell != 1){
                throw std::runtime_error("This is not an element with one edge on physical boundary.");
            ...
            for(it_cell = cells_in_patch.begin();it_cell!=cells_in_patch.end();++it_cell){
                ...
                if(icell->active_cell_index() == cell->active_cell_index()){
                    ...
                    for(unsigned int inei = 0; inei < local_face_indices.size(); ++inei){
                        if(icell->active_cell_index() == cell->neighbor_index(local_face_indices[inei])){
            ...
            if(cells_in_patch.size() == 4){
                ...
                if(faces_on_boundary.size() != 2){
                    throw std::runtime_error("This is not an element with two edges on physical boundary.");
                }
                ...
                for(it_cell = cells_in_patch.begin();it_cell!=cells_in_patch.end();++it_cell){
                    ...
                    if(icell->active_cell_index() == cell->active_cell_index()){
                        ...
                        for(unsigned int inei = 0; inei < f_n_b.size(); ++inei){
                            if(icell->active_cell_index() == cell->neighbor_index(f_n_b[inei])){
                                cell_patch_map.insert({1+inei,icell});
                                neighbour_cell = true;
                            }
                        }
...
template<int dim, int spacedim>
std::vector<std::vector<types::global_dof_index>> Catmull_Clark<dim,spacedim>::new_dofs_for_cells(){
    std::vector<std::vector<types::global_dof_index>> global_dof_vector_vector(dof_handler.get_triangulation().n_active_cells());
    for (auto cell = dof_handler.begin_active(); cell != dof_handler.end(); ++cell){
        ...
        switch (cells.size()){
            ...
            default:
                ...
                std::vector<types::global_dof_index> new_global_dof_indices(2*valence+8,0);
...
template <int dim, int spacedim>
std::vector<unsigned int> Catmull_Clark<dim,spacedim>::get_neighbour_dofs(typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell_0, typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell_neighbour){
...
template<int dim, int spacedim>
const std::array<unsigned int,4> Catmull_Clark<dim,spacedim>::vertex_face_loop(const unsigned int local_vertex_id){
...
template<int dim, int spacedim>
unsigned int Catmull_Clark<dim,spacedim>::opposite_vertex_dofs(unsigned int i){
    switch(i){
        case 0:
            return 3;
        case 1:
...
template <int dim, int spacedim>
std::vector<unsigned int> Catmull_Clark<dim,spacedim>::opposite_face_dofs(unsigned int i, unsigned int j){
...
template <int dim, int spacedim>
const std::array<unsigned int,3> Catmull_Clark<dim,spacedim>::next_vertices(const unsigned int local_vertex_id) {
    switch(local_vertex_id)
    {
        case 0:
            return {1,3,2};
...
template <int dim, int spacedim>
const std::array<unsigned int,3> Catmull_Clark<dim,spacedim>::loop_faces(const unsigned int local_face_id){
...
template <int dim, int spacedim>
const std::array<unsigned int,2> Catmull_Clark<dim,spacedim>::opposite_vertices(const unsigned int local_face_id){
    switch(local_face_id)
    {
        case 0:
            return {3,1};
...
template <int dim, int spacedim>
const std::array<unsigned int,2> Catmull_Clark<dim,spacedim>::faces_not_on_boundary(const std::vector<unsigned int> m){
    /*
     *       2
     *    0-----1
     *    |     |
...
template <int dim, int spacedim>
const std::array<unsigned int,4> Catmull_Clark<dim,spacedim>::verts_id_on_boundary(const std::vector<unsigned int> m){
    /*
     *       2
     *    0-----1
     *    |     |
...
template <int dim, int spacedim>
unsigned int Catmull_Clark<dim,spacedim>::common_face_local_id(typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell_0, typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell){
...
template <int dim, int spacedim>
std::array<unsigned int,4> Catmull_Clark<dim,spacedim>::rotated_vertices(const unsigned int local_face_id){
    switch(local_face_id)
    {
        case 0:
            return {0,2,3,1};
...
template <int dim, int spacedim>
unsigned int Catmull_Clark<dim,spacedim>::get_diagonal_dof_id_to_ex(typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell_0, typename hp::DoFHandler<dim,spacedim>::active_cell_iterator cell_neighbour, unsigned int ex_index){
    std::vector<types::global_dof_index> cell_0_indices(cell_0->get_fe().dofs_per_cell,0);
    std::vector<types::global_dof_index> cell_neighbour_indices(cell_neighbour->get_fe().dofs_per_cell,0);
    cell_0->get_dof_indices(cell_0_indices);
    cell_neighbour->get_dof_indices(cell_neighbour_indices);
...
template class Catmull_Clark<2,3>;
