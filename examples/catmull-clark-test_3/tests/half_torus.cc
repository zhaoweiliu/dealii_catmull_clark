...
public:
    ...
    Tensor<2, dim, Tensor<1,spacedim>> get_reference_covariant_bases_deriv()
    {
        return da_cov_ref;
    }
...
//    const double mu = 4.225e5;
    ...
    const double reference_pressure = 1000;
...
Triangulation<dim,spacedim> set_mesh( std::string type )
{
    ...
    }else if (type == "plate")
    {
        ...
        grid_in.read_msh(file);
        mesh.refine_global(1);
    }else if (type == "torus_plate")
    {
        std::string mfile = "/Users/zhaoweiliu/Documents/geometries/torus_plate.msh";
        GridIn<2,3> grid_in;
        grid_in.attach_triangulation(mesh);
        std::ifstream file(mfile.c_str());
        Assert(file, ExcFileNotOpen(mfile.c_str()));
        grid_in.read_msh(file);
        GridTools::scale(10./ 21.8068, mesh);
//        mesh.refine_global(1);
...
//        PointHistory<dim,spacedim> *lqph = reinterpret_cast<PointHistory<dim,spacedim>*>(cell->user_pointer());
             ...
             ++q_point)
        {
            ...
            double factor_material = 1.;
            if(std::abs(qpt_ref[1])<tolerance ){
                factor_material = 0.000001;
            }
            // covariant base  a_1, a_2, a_3;
...
//                lqph[q_point].setup_cell_qp(thickness, a_cov_ref, da_cov_ref, mu);
            ...
            if (first_load_step == false || first_newton_step == false)
            {
                lqph[q_point].update_cell_qp(delta_u_der,delta_u_der2);
            ...
            for (unsigned int r_shape = 0; r_shape < dofs_per_cell; ++r_shape) {
                ...
                for (unsigned int s_shape = 0; s_shape < dofs_per_cell; ++s_shape) {
                    ...
                    for (unsigned int ia = 0; ia < dim; ++ia) {
                        for (unsigned int ib = 0; ib < dim; ++ib) {
                            cell_tangent_matrix[r_shape][s_shape] += (membrane_strain_drs[ia][ib] * resultants[0][ia][ib] + bending_strain_drs[ia][ib] * resultants[1][ia][ib]) * fe_values.JxW(q_point) * factor_material;
                            for (unsigned int ic = 0; ic < dim; ++ic) {
                                for (unsigned int id = 0; id < dim; ++id) {
                                    cell_tangent_matrix[r_shape][s_shape] += (membrane_strain_dr[ia][ib] * D0[ia][ib][ic][id] * membrane_strain_ds[ic][id]
                                                                            ...
                                                                            * fe_values.JxW(q_point)* factor_material;
                                }
                            }
                        }
                    }
                    // following pressure load
                    cell_tangent_matrix[r_shape][s_shape] -= (lambda + pressure_increment_load_step) * reference_pressure * scalar_product(a3_t_s, u_r) * (1./detJ_ref) * fe_values.JxW(q_point)* factor_material;
                }
                for (unsigned int ia = 0; ia < dim; ++ia) {
                    for (unsigned int ib = 0; ib < dim; ++ib) {
                        cell_internal_force_rhs[r_shape] += (membrane_strain_dr[ia][ib] * resultants[0][ia][ib] + bending_strain_dr[ia][ib] * resultants[1][ia][ib]) * fe_values.JxW(q_point)* factor_material; // f^int
                    }
                }
                if(qpt_ref[1] > 0){
            ...
            volume += std::abs(fe_values.quadrature_point(q_point)[1]) * (detJ_def/detJ_ref) * fe_values.JxW(q_point);
        ...
        for (unsigned int ivert = 0; ivert < GeometryInfo<dim>::vertices_per_cell; ++ivert){
            double xx = cell->vertex(ivert)[0];
            double yy = cell->vertex(ivert)[1];
            double zz = cell->vertex(ivert)[2];
            if (std::abs(xx - 10.) < tolerance && std::abs(zz) < tolerance) {
                unsigned int dof_id = cell->vertex_dof_index(ivert,0, cell->active_fe_index());
                constrained_dof_indices.push_back(dof_id + 2);
                
            }
            else if(std::abs(xx) < tolerance && std::abs(zz - 10.) < tolerance  ){
                unsigned int dof_id = cell->vertex_dof_index(ivert,0, cell->active_fe_index());
...
//        pressure_increment_load_step = 0.1;
        ...
        vtk_plot("torus_plate= "+std::to_string(step)+".vtu", dof_handler, mapping_collection, vec_values, present_solution, Vector<double>(), lambda * reference_pressure,area,volume);
...
//            reduced_full_tangent.compute_eigenvalues_symmetric(-10000, 50, 1e-5, eigenvalues, eigenvectors);
...
//                    ++ibf;
//                    if (ibf > 1) {
//                        vtk_plot("torus_eigen_"+std::to_string(eigenvalues[ie])+".vtu", dof_handler, mapping_collection, vec_values, restored_eigenvec);
//                        std::cout <<"norm of eigenvector = "<< restored_eigenvec.l2_norm()<<endl;
//                        bifurcation_disp = 5. * restored_eigenvec;
//                        ++bifurcation_number;
//                        bifurcation = true;
//                    }
//                }
...
//            vtk_plot("torus_eigen_1.vtu", dof_handler, mapping_collection, vec_values, eigenvecs[0]);
//            vtk_plot("torus_eigen_2.vtu", dof_handler, mapping_collection, vec_values, eigenvecs[1]);
...
int main()
{
    ...
    Triangulation<dim,spacedim> mesh = set_mesh<dim,spacedim>("torus_plate");
