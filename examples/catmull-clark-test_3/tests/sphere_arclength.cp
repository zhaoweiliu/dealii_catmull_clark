...
double Nonlinear_shell<dim, spacedim> :: get_error_residual(){
    std::cout << "constrained_dof_indices number: "<< constrained_dof_indices.size()<<std::endl;
    Vector<double> rhs = force_rhs;
    for (unsigned int ic = 0; ic < constrained_dof_indices.size(); ++ic) {
        rhs[constrained_dof_indices[ic]] = 0;
    }
    return rhs.l2_norm();
}
...
template <int dim, int spacedim>
void Nonlinear_shell<dim, spacedim>::arclength_system_rhs()
{
    double B = 2 * beta * beta * delta_alpha * (p_vector_rhs * p_vector_rhs);
    double C = radius * radius - delta_u * delta_u - beta * beta * (delta_alpha * delta_alpha) * (p_vector_rhs * p_vector_rhs);
    force_rhs += C/B * p_vector_rhs;
}
...
void Nonlinear_shell<dim, spacedim>::solve()
{
    ...
    FullMatrix<double> PVT;
    Vector<double> V = 2 * delta_u;
    PVT.outer_product(p_vector_rhs, V);
    const auto op_pvt = linear_operator(PVT);
    double B = 2 * beta * beta * delta_alpha * (p_vector_rhs * p_vector_rhs);
    double C = radius * radius - delta_u * delta_u - beta * beta * (delta_alpha * delta_alpha) * (p_vector_rhs * p_vector_rhs);
    const auto op_s = op_k + 1./B * op_pvt;
    ...
    double D = V * newton_update_u;
    newton_update_alpha = C/B - D/B;
    std::cout << "B = " << B << "; C = " << C << "; D = " << D <<"; newton_update_alpha = " << newton_update_alpha <<std::endl;
}
...
void Nonlinear_shell<dim, spacedim> ::nonlinear_solver(const bool initial_load_step){
    ...
    {
        std::cout << "newton_iteration: " << std::setw(2) << newton_iteration << " " << std::endl;
