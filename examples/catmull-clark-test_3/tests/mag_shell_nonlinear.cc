...
//                }
                if (r_shape%3 == 0){
                    cell_external_force_rhs[r_shape] += f_load * 10 * thickness * shape_r * fe_values.JxW(q_point);
                        }
                for (unsigned int ia = 0; ia < dim; ++ia) {
                    cell_external_force_rhs[r_shape] +=  - thickness / mu_0 * (a_contra_ref[ia] * residual_mag) * (a_cov_ar[ia] * applied_mag ) * (detJ_def/detJ_ref) * fe_values.JxW(q_point);
                }
                cell_external_force_rhs[r_shape] +=  - thickness / mu_0 * (a_contra_ref[2] * residual_mag) * (a3_dr * applied_mag ) * (detJ_def/detJ_ref) * fe_values.JxW(q_point);
//                cell_external_force_rhs[r_shape] +=  - thickness / mu_0 * (a_cov_ref[2] * residual_mag) * (a3_dr * applied_mag ) * fe_values.JxW(q_point);
            }//loop r_shape
        ...
        for (unsigned int ivert = 0; ivert < GeometryInfo<dim>::vertices_per_cell; ++ivert)
        {
            ...
            if (cell->vertex(ivert)[0] == 25.2e-3/19. )
            {
                unsigned int dof_id = cell->vertex_dof_index(ivert,0, cell->active_fe_index());
                fix_dof_indices.push_back(dof_id);
                fix_dof_indices.push_back(dof_id+1);
...
//        GridGenerator::hyper_cube<dim,spacedim>(mesh,0,25.2e-3);
        GridGenerator::subdivided_hyper_rectangle(mesh,{20,20}, {0,0}, {25.2e-3*(20./19.),25.2e-3});
