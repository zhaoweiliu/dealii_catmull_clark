...
//                if (arc_length == true) {
                cell_vt_vector[r_shape] += 2 * u_def[r_shape%3] * shape_r * fe_values.JxW(q_point);
//                }
            }
            area += fe_values.JxW(q_point);
//            if (arc_length == true){
            B += 2 * beta * beta * delta_alpha * f_load_reference * f_load_reference * fe_values.JxW(q_point);
            C += (radius * radius - scalar_product(u_def, u_def)  - beta * beta * delta_alpha * delta_alpha * f_load_reference * f_load_reference) * fe_values.JxW(q_point);
//            }
...
{
    setup_system();
    for (unsigned int step = 0; step < 50; ++step)
    {
        delta_u.reinit(dof_handler.n_dofs());
        if (step < 1){
            delta_alpha = 0.1;
        ...
        vtk_plot("refine_sphere = "+std::to_string(step)+".vtu", dof_handler, mapping_collection, vec_values, present_solution_u);
    }
}
...
void Nonlinear_shell<dim, spacedim> ::nonlinear_solver(const bool initial_load_step){
    ...
    for (unsigned int newton_iteration = 0; newton_iteration < max_newton_iteration; ++ newton_iteration)
    {
        ...
        if ((residual_error < 1e-3 ) && newton_update_u.l2_norm() < 1e-9) {
            std::cout << "converged.  ";
