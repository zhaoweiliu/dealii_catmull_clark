...
//                cell_external_force_rhs[r_shape] += reference_pressure * scalar_product(a_cov_def[2], u_r)  * fe_values.JxW(q_point); //  f^ext;
...
//            if(step == 6){
//                pressure_increment_load_step = 0.01;
//            }
        }
...
//            std::cout<< " radius = " << radius <<std::endl;
            std::cout<< " displacement_step_length = " << std::sqrt(psi_2 * VTV(solution_increment_load_step))  << std::endl;
            std::cout << " load_step_length = " <<  std::sqrt(psi_1 * lambda * lambda * reference_pressure_VTV) << std::endl;
        }else if( step > 5)
        {
            radius = std::sqrt(psi_2 * VTV(solution_increment_load_step));
//            std::cout<< " radius = " << radius <<std::endl;
            std::cout<< " displacement_step_length = " << std::sqrt(psi_2 * VTV(solution_increment_load_step))  << std::endl;
            std::cout << " load_step_length = " <<  std::sqrt(psi_1 * pressure_increment_load_step * pressure_increment_load_step * reference_pressure_VTV) << std::endl;
        }
        else
        {
...
//        radius = 0.1;
        ...
        std::cout << "pressure_load = " << lambda * reference_pressure /* 0.5 * M_PI / area*/  << "n/m2" <<std::endl;
        
        vtk_plot("arclength_sphere_= "+std::to_string(step)+".vtu", dof_handler, mapping_collection, vec_values, present_solution, Vector<double>(), lambda * reference_pressure /* 0.5 * M_PI / area*/);
...
//        }
        ...
        if ((residual_error < 2e-2 ) && solution_newton_update.l2_norm() < 1e-6) {
            std::cout << "converged.\n";
            tangent_matrix.reinit(sparsity_pattern);
