...
//    std::vector<PointHistory_MR<dim,spacedim>>  quadrature_point_history;
    ...
    Vector<double> solution_increment_step;
    double increment_alpha;
    ...
    const double reference_load = 3000;
    double B, C, alpha, beta = 1e-4, delta_alpha;
    double radius = 0.5;
...
//    constraints.clear();
    ...
    solution_increment_step.reinit(dof_handler.n_dofs());
    increment_alpha = 0;
    B, C, alpha = 0;
}
...
//        PointHistory_MR<dim,spacedim> *lqph = reinterpret_cast<PointHistory_MR<dim,spacedim>*>(cell->user_pointer());
             ...
             ++q_point)
        {
            ...
            for (unsigned int r_shape = 0; r_shape < dofs_per_cell; ++r_shape) {
                ...
                cell_external_force_rhs[r_shape] += reference_load * scalar_product(a_cov_def[2], u_r) * fe_values.JxW(q_point); // p = 1. f^ext;
            }
    ...
    force_rhs = alpha * external_force_rhs - internal_force_rhs;
}
...
{
  FullMatrix<double> DMatrix;
  DMatrix.outer_product(external_force_rhs, solution_increment_step);
  double B = 2 * beta * beta * delta_alpha * external_force_rhs * external_force_rhs;
  double C = -( solution_increment_step * solution_increment_step + beta * beta * delta_alpha * delta_alpha * (external_force_rhs * external_force_rhs) - radius * radius);
  const auto op_k = linear_operator(tangent_matrix);
  const auto op_d = linear_operator(DMatrix);
  const auto op_s = op_k + 2/B * op_d;
  force_rhs.add(C/B, external_force_rhs);
  ...
  newton_update = op_s_inv * force_rhs;
  double D = 2 * solution_increment_step * newton_update;
...
{    setup_system();
    ...
    for (unsigned int step = 0; step < 50; ++step) {
        ...
        double p_t = alpha * reference_load;
        vtk_plot("refine_sphere = "+std::to_string(step)+".vtu", dof_handler, mapping_collection, vec_values, present_solution, Vector<double>(), p_t);
    }
}
...
//            residual_norm = get_error_residual();
        ...
        solution_increment_step += newton_update;
