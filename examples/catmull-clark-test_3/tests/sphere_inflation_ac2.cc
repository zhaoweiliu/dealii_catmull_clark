...
//                lqph[q_point].setup_cell_qp(thickness, a_cov_ref, da_cov_ref, mu);
            ...
            for (unsigned int r_shape = 0; r_shape < dofs_per_cell; ++r_shape) {
                ...
                for (unsigned int s_shape = 0; s_shape < dofs_per_cell; ++s_shape) {
                    ...
                    if(r_shape % 3 == s_shape % 3){
                        cell_mass_matrix[r_shape][s_shape] = shape_s * shape_r * fe_values.JxW(q_point);
                    }
    ...
    a_vector = 2 * solution_increment_load_step;
    b = 2 * psi * pressure_increment_load_step * VTV(external_force_rhs);
    A = VTV(solution_increment_load_step) + psi * pressure_increment_load_step * pressure_increment_load_step * VTV(external_force_rhs) - radius * radius;
...
//        mass_matrix.add(penalty_factor, boundary_mass_matrix);
...
//                tangent_matrix.set(constrained_dof_indices[idof], constrained_dof_indices[idof], 1e50);
//                mass_matrix.set(constrained_dof_indices[idof], constrained_dof_indices[idof], 1e50);
//
//            }
//            else
//            {
...
//                mass_matrix.set(constrained_dof_indices[idof], jdof, 0);
//                mass_matrix.set(jdof, constrained_dof_indices[idof], 0);
//            }
//        }
//    }
...
//    full_tangent.compute_lu_factorization();
//    std::cout << "determinant of tangent_matrix = " << full_tangent.determinant()<<std::endl;
...
{   setup_system();
    ...
    for (unsigned int step = 0; step < max_load_step; ++step) {
        ...
        {
            radius = std::sqrt(VTV(solution_increment_load_step) + psi * pressure_increment_load_step * pressure_increment_load_step * reference_pressure_VTV);
            std::cout<< " radius = " << radius <<std::endl;
            std::cout<< " displacement_step_length = " << std::sqrt(VTV(solution_increment_load_step))  << std::endl;
...
void Nonlinear_shell<dim, spacedim> ::nonlinear_solver(const bool first_load_step){
    ...
    for (unsigned int newton_iteration = 0; newton_iteration < max_newton_step; ++ newton_iteration)
    {
        ...
        if (newton_iteration == 0) {
            first_newton_step = true;
        }else{
            first_newton_step = false;
        }
        ...
        LAPACKFullMatrix<double> full_mass(dof_handler.n_dofs());
//
        full_tangent = tangent_matrix;
        full_mass = mass_matrix;
//        std::vector<Vector<double> >
//        eigenvectors(dof_handler.n_dofs());
//        std::vector<std::complex<double>>   eigenvalues(dof_handler.n_dofs());
//        full_tangent.compute_generalized_eigenvalues_symmetric(full_mass, eigenvectors);
        FullMatrix<double>       eigenvectors;
        Vector<double>           eigenvalues;
        Vector<double>           eigenvec(dof_handler.n_dofs());
        std::vector<Vector<double>>           eigenvecs(0);

//        full_tangent.compute_eigenvalues_symmetric(0, 10000, 1e-5, eigenvalues, eigenvectors);
//        for(unsigned int ie = 0; ie < eigenvalues.size(); ++ie){
////            eigenvalues[ie] = full_tangent.eigenvalue(ie);
