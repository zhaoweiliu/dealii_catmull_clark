...
private:
    ...
    Tensor<2,dim> am_cov_def;
    // contravariant metric tensor(deformed)
    Tensor<2,dim> am_contra_def;
...
void Nonlinear_shell<dim, spacedim> ::run()
{
    ...
    for (unsigned int step = 0; step < max_load_step; ++step) {
        ...
        vtk_plot("1_plate"+std::to_string(step)+".vtu", dof_handler, mapping_collection, vec_values, present_solution);
    }
...
//        cell_b_mass_matrix = 0; cell_b_rhs = 0;
        ...
        if (b_fe_values.n_quadrature_points != 1)
        {
                 ...
                 ++q_point)
            {
                ...
                double tol = 1e-9;
//                if (std::abs(qpt[0]) < tol || std::abs(qpt[0] - 100.) < tol) {
//                    double jxw;
...
//                                if ( (std::abs(qpt[0]) < tol || std::abs(qpt[0] - 100.) < tol) && b_fe_values.shape_value(j_shape, q_point) > tol) {
...
//            double detJ_def = cross_product_3d(a_cov_def[0], a_cov_def[1]).norm();
            ...
            for (unsigned int r_shape = 0; r_shape < dofs_per_cell; ++r_shape) {
                ...
                if(r_shape%3 == 2){
                    cell_external_force_rhs[r_shape] += - 1 * shape_r * fe_values.JxW(q_point); // f_z = -1
                }
            }//loop r_shape
