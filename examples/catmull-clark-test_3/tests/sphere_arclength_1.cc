...
private:
    ...
    double newton_update_alpha;
    Vector<double> present_solution_u;
    double present_solution_alpha;
    Vector<double> solution_increment_u;
    Vector<double> delta_u;
    double solution_increment_alpha;
    double delta_alpha;
    Vector<double> internal_force_rhs;
    Vector<double> external_force_rhs;
    Vector<double> force_rhs;
    Vector<double> p_vector_rhs;
    Vector<double> boundary_value_rhs;
    Vector<double> boundary_edge_load_rhs;
    
    FullMatrix<double> p_vector;
    FullMatrix<double> vt_vector;
    double B, C;
...
//    double load_factor;
    ...
    double radius = 100, beta = 0.2, gamma = 1;
...
void Nonlinear_shell<dim, spacedim> :: setup_system()
{
    ...
    solution_increment_u.reinit(dof_handler.n_dofs());
    delta_u.reinit(dof_handler.n_dofs());
    solution_increment_alpha = 0;
    internal_force_rhs.reinit(dof_handler.n_dofs());
    external_force_rhs.reinit(dof_handler.n_dofs());
    present_solution_u.reinit(dof_handler.n_dofs());
    present_solution_alpha = 0.;
    newton_update_u.reinit(dof_handler.n_dofs());
    newton_update_alpha = 0;
    boundary_value_rhs.reinit(dof_handler.n_dofs());
    boundary_edge_load_rhs.reinit(dof_handler.n_dofs());
    
    p_vector.reinit(dof_handler.n_dofs(),1);
    vt_vector.reinit(1,dof_handler.n_dofs());
    p_vector_rhs.reinit(dof_handler.n_dofs());
    B = 0;
    C = 0;
}
...
void Nonlinear_shell<dim, spacedim> :: assemble_system(const bool initial_step)
{
    ...
    Vector<double>     cell_p_vector;
    Vector<double>     cell_vt_vector;
...
//    bool first_element = true, arc_length = false;
    double area = 0;
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
//        if (first_element == true && cell->at_boundary() == false) {
//            arc_length = true;
//            first_element = false;
//        }else{
//            arc_length = false;
//        }
        ...
        cell_p_vector.reinit(dofs_per_cell);
        cell_p_vector = 0;
        cell_vt_vector.reinit(dofs_per_cell);
        cell_vt_vector = 0;
             ...
             ++q_point)
        {
            ...
            Tensor<1, spacedim> u_def;
            Tensor<1, dim, Tensor<1,spacedim>> Du_der; // u_{,a}
            Tensor<2, dim, Tensor<1,spacedim>> Du_der2; // u_{,ab}
            ...
            for (unsigned int i_shape = 0; i_shape < dofs_per_cell; ++i_shape) {
                ...
                u_def[i_shape%3] += i_shape_vlaue * delta_u(local_dof_indices[i_shape]); // u = sum N^A * U_A
                for (unsigned int ia = 0; ia < dim; ++ia){
                    Du_der[ia][i_shape%3] += i_shape_der[ia] * solution_increment_u(local_dof_indices[i_shape]); // u_{,a} = sum N^A_{,a} * U_A
                    for (unsigned int ib = 0; ib < dim; ++ib){
                        Du_der2[ia][ib][i_shape%3] += i_shape_der2[ia][ib] * solution_increment_u(local_dof_indices[i_shape]); // u_{,ab} = sum N^A_{,ab} * U_A
                    }
                }
            }
            
            if (initial_step == false) {lqph[q_point].update_cell_qp(Du_der,Du_der2);}
            ...
            for (unsigned int r_shape = 0; r_shape < dofs_per_cell; ++r_shape) {
                ...
                cell_external_force_rhs[r_shape] += present_solution_alpha * f_load_reference * scalar_product(a_cov_def[2], u_r) * (detJ_def/detJ_ref) * fe_values.JxW(q_point); // p = alpha * f^ext;
                
                cell_p_vector[r_shape] += f_load_reference * scalar_product(a_cov_def[2], u_r) * (detJ_def/detJ_ref) * fe_values.JxW(q_point);
//                if (arc_length == true) {
                cell_vt_vector[r_shape] += 2 * gamma * gamma * u_def[r_shape%3] * shape_r * fe_values.JxW(q_point);
//                }
            }
            area += fe_values.JxW(q_point);
//            if (arc_length == true){
            B += 2 * beta * beta * delta_alpha * f_load_reference * f_load_reference * fe_values.JxW(q_point);
            C += (radius * radius - gamma * gamma * scalar_product(u_def, u_def)  - beta * beta * delta_alpha * delta_alpha * f_load_reference * f_load_reference) * fe_values.JxW(q_point);
//            }
        ...
        p_vector_rhs.add(local_dof_indices,cell_p_vector);
        for( unsigned int id = 0; id < local_dof_indices.size(); ++id){
            p_vector.add(local_dof_indices[id], 0, cell_p_vector[id]);
            vt_vector.add(0, local_dof_indices[id], cell_vt_vector[id]);
        }
    } // loop over cells
    // K \Delta u = -R
...
void Nonlinear_shell<dim, spacedim>::make_constrains(const unsigned int newton_iteration){
    ...
    force_rhs += boundary_edge_load_rhs;
...
void Nonlinear_shell<dim, spacedim>::solve()
{
    ...
    const auto op_p = linear_operator(p_vector);
    const auto op_vt = linear_operator(vt_vector);
    const auto op_s = op_k + 1./B * op_p * op_vt;
    SolverControl            solver_control(20000, 1e-9);
    SolverCG<Vector<double>> solver(solver_control);
    PreconditionSSOR<SparseMatrix<double>> preconditioner;
    preconditioner.initialize(tangent_matrix);
//    solver.solve(tangent_matrix, newton_update_u, force_rhs, preconditioner);
    const auto op_s_inv = inverse_operator(op_s, solver, preconditioner);
    newton_update_u = op_s_inv * force_rhs;
    double D = 0;
    for (unsigned int id = 0; id < dof_handler.n_dofs(); ++id) {
        D += vt_vector[0][id] * newton_update_u[id];
    }
...
void Nonlinear_shell<dim, spacedim> ::run()
{
    ...
    for (unsigned int step = 0; step < 30; ++step)
    {
        ...
        present_solution_alpha += delta_alpha;
        present_solution_u += delta_u;
        if (step == 0) {
            nonlinear_solver(true);
        }else{
            nonlinear_solver();
        }
        double p_t = present_solution_alpha * f_load_reference;
        vtk_plot("refine_sphere = "+std::to_string(step)+".vtu", dof_handler, mapping_collection, vec_values, present_solution_u, Vector<double>(), p_t);
    }
}
...
void Nonlinear_shell<dim, spacedim> ::nonlinear_solver(const bool initial_load_step){
    ...
    for (unsigned int newton_iteration = 0; newton_iteration < max_newton_iteration; ++ newton_iteration)
    {
        ...
        force_rhs.add(C/B, p_vector_rhs);
        if (newton_iteration == 0) {
            std::cout << "first newton iteration " << std::endl;
            residual_error = 1;
        }else if (newton_iteration == 1)
        {
            initial_residual = get_error_residual();
            residual_error = 1;
            std::cout << "residual = " << initial_residual << std::endl;
        }else{
        ...
        if ((residual_error < 1e-2 ) && newton_update_u.l2_norm() < 1e-9) {
            std::cout << "converged.  ";
            std::cout << "f_load = " << present_solution_alpha * f_load_reference << ", present_solution_alpha = "<< present_solution_alpha <<std::endl;
            std::cout << "delta_load_factor = " << delta_alpha  <<std::endl;
            std::cout << "delta_u^2 = " << delta_u * delta_u <<std::endl;
            tangent_matrix.reinit(sparsity_pattern);
            internal_force_rhs.reinit(dof_handler.n_dofs());
            external_force_rhs.reinit(dof_handler.n_dofs());
            newton_update_u.reinit(dof_handler.n_dofs());
        ...
        std::cout << "newton_update_u = " << newton_update_u.l2_norm() <<std::endl;
        std::cout << "newton_update_alpha = " << newton_update_alpha <<std::endl;

        solution_increment_u = newton_update_u;
        delta_u += newton_update_u;
//        present_solution_u += newton_update_u;
        solution_increment_alpha = newton_update_alpha;
        delta_alpha += newton_update_alpha;
//        present_solution_alpha += newton_update_alpha;
