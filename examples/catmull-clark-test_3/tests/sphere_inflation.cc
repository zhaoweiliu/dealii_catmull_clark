...
//    std::vector<PointHistory_MR<dim,spacedim>>  quadrature_point_history;
    ...
    Vector<double> solution_increment_newton;
...
//    constraints.clear();
    ...
    solution_increment_newton.reinit(dof_handler.n_dofs());
...
//                lqph[q_point].setup_cell_qp(thickness, a_cov_ref, da_cov_ref, c_1, c_2);
            ...
            for (unsigned int i_shape = 0; i_shape < dofs_per_cell; ++i_shape) {
                ...
                for (unsigned int ia = 0; ia < dim; ++ia){
                    delta_u_der[ia][i_shape%3] += i_shape_der[ia] * solution_increment_newton(local_dof_indices[i_shape]); // u_{,a} = sum N^A_{,a} * U_A
                    for (unsigned int ib = 0; ib < dim; ++ib){
                        delta_u_der2[ia][ib][i_shape%3] += i_shape_der2[ia][ib] * solution_increment_newton(local_dof_indices[i_shape]); // u_{,ab} = sum N^A_{,ab} * U_A
                    }
                }
            }
            
            if (initial_step == false) {lqph[q_point].update_cell_qp(delta_u_der,delta_u_der2);}
...
{    setup_system();
    for (unsigned int step = 0; step < pressure.size(); ++step) {
        solution_increment_step = 0;
        std::cout << "step = "<< step << std::endl;
        bool vtk = false;
        f_load = pressure[step];
        if(step == 0){
        ...
        present_solution += solution_increment_step;
        std::cout << "f_load = "<< f_load <<std::endl;
        if (vtk == true)
            vtk_plot("refine_sphere_f_load = "+std::to_string(f_load)+".vtu", dof_handler, mapping_collection, vec_values, present_solution,Vector<double>(), pressure[step] );
    }
}
...
void Nonlinear_shell<dim, spacedim> ::nonlinear_solver(const bool initial_load_step){
    ...
    for (unsigned int newton_iteration = 0; newton_iteration < max_newton_iteration; ++ newton_iteration)
    {
        ...
        solution_increment_newton = newton_update;
