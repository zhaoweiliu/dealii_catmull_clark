...
#include <addition_lib/include/Catmull_Clark_Data.hpp>
#include <addition_lib/include/polynomials_Catmull_Clark.hpp>
#include <addition_lib/include/FE_Catmull_Clark.hpp>
#include <addition_lib/include/MappingFEField_hp.hpp>
...
//    vtk_plot("coupled_shell_solution.vtu", dof_handler, mapping_collection, vec_values, solution_disp_coupled, potential);
    ...
    // eigenvalue problem
    std::vector<Vector<double> >        eigenvectors(dof_handler.n_dofs()+1);
    std::vector<std::complex<double>>   eigenvalues(dof_handler.n_dofs());
    for (unsigned int i = 0; i < eigenvectors.size(); ++i){
        eigenvectors[i].reinit(dof_handler.n_dofs()+1);
    }
//    SolverControl solver_control(dof_handler.n_dofs(), 1e-9);
//    SparseDirectUMFPACK inverse;
//    ArpackSolver::AdditionalData additional_data(true);
//    inverse.initialize (stiffness_matrix);
//    ArpackSolver solver(solver_control);
//    solver.solve(stiffness_matrix, mass_matrix, inverse, eigenvalues, eigenvectors);
//    vtk_plot("elastic_shell_eigen_solution.vtu", dof_handler, mapping_collection, eigenvectors[0], solution_disp);
    return 0;
}
